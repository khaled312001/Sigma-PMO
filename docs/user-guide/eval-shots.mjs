/** Proof screenshots for the eval-92 response: P-1000 pages now populated + Swagger. */
...
const PAGES = [
  ['risk', '/risk'], ['claims', '/claims'], ['quantity-survey', '/quantity-survey'],
  ['knowledge', '/knowledge'], ['sources', '/sources'], ['review', '/review'],
  ['predictive', '/predictive'], ['analytics', '/analytics'], ['decisions', '/decisions'],
];

async function login() { for (let i = 0; i < 5; i++) { const r = await fetch(`${API}/auth/login`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ email: 'admin@sigma.local', password: ADMIN_PW }) }); if (r.status === 429) { await new Promise(x => setTimeout(x, 12000)); continue; } if (r.ok) return (await r.json()).apiKey; return null; } }
...
// Swagger UI (public) — show the documented API
