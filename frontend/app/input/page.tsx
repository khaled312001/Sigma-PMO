...
import { Button, Card, ConfidenceBar, EmptyState, ErrorBanner, PageHeader, Pill } from '../../components/ui';
  ...
  return (
    <div className="space-y-7">
      <PageHeader
        ...
        description="Primavera P6 (.xer / .xml), Excel (.xlsx), or CSV. Each upload is content-addressed (SHA-256) and ingested through parse → validate → normalize → score."
      ...
      <Card title="Upload a file" hint="Drop here or browse. The file is archived immutably and traced through the entire pipeline.">
        ...
        >
          ...
          {file ? (
            ...
            <>
              <p className="text-sm text-slate-200">Drag a P6 / Excel / CSV file here</p>
        ...
        {runs.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-900/40 text-left text-[10px] uppercase tracking-wider text-slate-400">
              <tr><th className="px-5 py-2.5">When</th><th className="py-2.5">Parser</th><th className="py-2.5">Status</th><th className="py-2.5">Counts</th><th className="py-2.5 pr-5">Confidence</th></tr>
            </thead>
            <tbody>
              {runs.map((r) => {
